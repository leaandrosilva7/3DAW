...
(function () {
  ...
  var UNITS = [];
  var SERVICES = [];
  var catalogoStatus = "idle"; // idle | loading | ready | error
  var catalogoPromise = null;
 
  function loadCatalogo() {
    if (catalogoPromise) return catalogoPromise;
    catalogoStatus = "loading";
    catalogoPromise = fetch("api/catalogo.php", { credentials: "include" })
      .then(function(resp) {
        if (!resp.ok) throw new Error("HTTP " + resp.status);
        return resp.json();
      })
      .then(function(data) {
        if (!data.success) throw new Error(data.mensagem || "Falha ao carregar catalogo.");
        UNITS = data.unidades;
      ...
      .catch(function(err) {
        ...
        catalogoStatus = "error";
        throw err;
      });
    return catalogoPromise;
  }
 
  // dispara o carregamento assim que o script roda, pra já estar
  // pronto (ou quase) quando o usuario clicar em "Agendar"
  loadCatalogo();
  ...
  var usuarioLogado  = window.currentUser || null;
  const CLIENTE_LOGADO = usuarioLogado ? usuarioLogado.nome : "";
  ...
  if (openBtn) {
    openBtn.addEventListener("click", function() {
      ...
      if (catalogoStatus === "ready") {
        render();
        return;
      }
 
      // ainda carregando (ou falhou antes) - mostra loading e espera
      box.innerHTML = topBarHTML(false) + '<div class="step-panel" style="padding:40px 0;text-align:center;">Carregando...</div>';
 
      loadCatalogo().then(function() {
        if (overlay.classList.contains("active")) render();
      }).catch(function() {
        if (overlay.classList.contains("active")) {
          box.innerHTML = topBarHTML(false)
            + '<div class="step-panel" style="padding:40px 0;text-align:center;">'
            + 'Não foi possível carregar os dados. <button class="btn btn-outline" id="retry-catalogo">Tentar novamente</button>'
            + '</div>';
          bindTopBar();
          var retryBtn = document.getElementById("retry-catalogo");
          if (retryBtn) retryBtn.addEventListener("click", function() {
            catalogoPromise = null; // forca nova tentativa
            openBtn.click();
          });
        }
      });
  ...
  function submitAgendamento(btn) {
    ...
    var payload = {
      ...
      unidade:      state.unidade.id,   // ID da unidade (ex: "barra") — banco agora usa unidade_id
      data:         state.data,
      hora:         state.hora,
      servicos:     state.servicos,     // ja e um array de IDs (ex: ["relax","vacuo"])
      pagamento:    state.pagamento,    // ja e o ID do pagamento (ex: "pix")
      cupom:        state.cupom || null
      // total NAO e mais enviado: o servidor recalcula com base no preco
      // atual de cada servico no banco (evita cliente forjar o total).
      // usuario_id tambem NAO e enviado: o servidor pega da sessao (api/agendamento.php)
    };
